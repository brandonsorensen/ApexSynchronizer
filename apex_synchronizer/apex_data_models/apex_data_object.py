...
class ApexDataObject(ABC):
    ...
    @classmethod
    def get_collection(cls, import_ids: Collection[Union[str, int]],
        ...
        :param import_ids: a collection ApexDataObject IDs
        :param token: an Apex access token
        :param session: an existing Apex session
        :param ignore_errors: whether to ignore errors when they occur,
            instead simply skipping that object and moving onto the
            next, or allow them to be raised.
...
class ApexUser(ApexDataObject, ABC):
    ...
    def __init__(self, import_org_id: int, first_name: str,
        ...
        email = email.strip()
        super().__init__(email.lower(), import_org_id)
